	...
	<ul role="menubar" class="navbar-blank navbar-nav navbar-site">
		<#list nav_items as nav_item>
			...
			<li class="${nav_item_css_class}" id="layout_${nav_item.getLayoutId()}" role="presentation">
				<a class="nav-link text-truncate px-4 ds_nav${nav_item_css_class}" ${nav_item_attr_has_popup} href="${nav_item.getURL()}" ${nav_item.getTarget()} role="menuitem"><span><@liferay_theme["layout-icon"] layout=nav_item_layout /> ${nav_item.getName()}</span></a>
