...
$(document).ready(function () {
    ...
    $("select[name=search-select-dropdown]").focus(function () {
    	if($(this).val() == 'all financial transactions'){
        	previous = 'transactions';
    	}else if($(this).val() == 'titles/items'){
    		previous = 'catalogs';
    	}else{
    	     previous = $(this).val();
    	}
...
function showShipmentJumpToResults(element,event){
	clearForm($('#search-shipments form'));
...
function showCatalogJumpToResults(element,event){
	clearForm($('#search-catalogs form'));
	if(element.attr('data-attr')=='short_author') {
		$('#search-catalogs form').find('#catalog_author').val(event.currentTarget.text);
	}
