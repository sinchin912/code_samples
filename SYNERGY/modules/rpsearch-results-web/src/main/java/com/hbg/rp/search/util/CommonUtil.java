...
public class CommonUtil implements ICommonUtil {
	...
	public String getAllReportingGroups() throws PortalException, SystemException {
		if(genericCacheHandler == null) {
			genericCacheHandler=new GenericCacheHandler();
		}
		List<String> allReportingGroups = genericCacheHandler.getAllReportingGroups();//ProductLocalServiceUtil.getAllReportingGroups();
	...
	@Override
	public Map<String, Product> getProductTitleMap(List<String> isbnList) throws SystemException {
		List<Product> productList = ProductLocalServiceUtil.getProducts(isbnList);
		Map<String, Product> productMap = new HashMap<>();
		for (Product product : productList) {
			productMap.put(product.getIsbn(), product);
		}
		return productMap;
	}
