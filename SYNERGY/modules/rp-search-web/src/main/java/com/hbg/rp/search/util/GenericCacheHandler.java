...
public class GenericCacheHandler implements IGenericCacheHandler {
	...
	public List<String> getAllReportingGroups() {
		List<PortalMapping> reportingGroupList = PortalMappingLocalServiceUtil.getAllReportingGroups();
		List<String> reportingGroups = ProductLocalServiceUtil.getAllReportingGroups();
		List<String> finalReportingGroups = new ArrayList<>();
		for (PortalMapping portalMapping : reportingGroupList) {
			if (reportingGroups.contains(portalMapping.getMappedValue())) {
				finalReportingGroups.add(portalMapping.getMappedValue());
			}
		}
		return finalReportingGroups;
	}
